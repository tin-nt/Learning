...
-- 2.Tìm sản phẩm có đơn vị tính là cay, quyen
select MASP, TENSP
from SANPHAM where DVT = 'cay' or DVT = 'quyen'
-- 3. Tìm sp khác nhau được bán ra trong 2006
select COUNT(distinct a.MASP)
from CTHD a, HOADON b
where a.SOHD = b.SOHD and year(b.NGHD) = '2006'
-- 4. In ra danh sach 3 khách hàng có doanh  số cao nhất
select top 3 DOANHSO
from KHACHHANG order by DOANHSO desc
-- 5. Tính tổng số lượng của từng sản phẩm bán ra 10/2006

select  a.MASP, sum(sl) SL
from CTHD a, HOADON b
where a.SOHD = b.SOHD and year(NGHD) = '2006' and month(NGHD) = '10'
group by a.MASP
